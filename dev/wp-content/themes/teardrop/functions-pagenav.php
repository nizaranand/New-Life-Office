...
function wp_pagenavi($paged=1,$before = '', $after = '') {
			...
			switch(intval($pagenavi_options['style'])) {
				...
				if(!empty($pages_text)) {
					...
					if ($start_page >= 2 && $pages_to_show < $max_page) {
						...
						echo '<a href="'.clean_url(get_pagenum_link()).'" class="first" title="&laquo;">&laquo;</a>';
					...
					for($i = $start_page; $i  <= $end_page; $i++) {						
						if($i == $paged) {
							$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['current_text']);
							$class=" active";
						} else {
							$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['page_text']);
							$class="";
						}
						echo '<a href="'.clean_url(get_pagenum_link($i)).'" class="page'.$class.'" title="'.$page_text.'">'.$page_text.'</a>';
					}
					...
					if ($end_page < $max_page) {
						...
						echo '<a href="'.clean_url(get_pagenum_link($max_page)).'" class="last" title="&raquo;">&raquo;</a>';
					}
					break;
				case 2;
