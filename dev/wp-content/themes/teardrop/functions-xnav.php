...
function x_wp_nav_menu($args=array()){
  if(!isset($args['walker'])) $args['walker']=new X_Walker_Nav_Menu($args);
...
class X_Walker_Nav_Menu extends Walker_Nav_Menu {
  ...
  private function child_of_active($element){
    if(count($this->stack)== 1) return true;
  ...
  private function parent_of_active($element){
    if(!isset($element->classes)) return false;
  ...
  function display_element($element, &$children_elements, $max_depth, $depth=0, $args, &$output){
    ...
    if($depth >= $this->args['min_depth'] && $this->child_of_active($element) || ($depth > $this->args['min_depth'] && $this->args['show_children_of_inactive'] && $this->child_of_active_invisible()))
      parent::display_element($element, $children_elements, $max_depth, $depth, $args, $output);
    elseif($this->parent_of_active($element))
  ...
  function start_el(&$output, $item, $depth, $args){
    return $this->depth_is_acceptable($depth) ? parent::start_el($output, $item, $depth, $args) : '';
  }
