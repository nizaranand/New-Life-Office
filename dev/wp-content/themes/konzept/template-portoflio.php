...
/* function fg_imgsetbyordcache(orderside){
    if(fg_preview){
        var fg_oldimgsrc = $("#fg_preview .imgdisplay .imgprevcontainer img").attr("src");
        var fg_oldimgcachei = -1;
        for(var fg_l=0;fg_l<fg_imagesthumbs_cache.length;fg_l++){
            if(fg_imagesthumbs_cache[fg_l].src == fg_oldimgsrc){
                fg_oldimgcachei = fg_l;
                break;
            }
        }
        if(fg_oldimgcachei == -1){
            fg_imgchangepreview(fg_imagesthumbs_cache[0].src);
        }else{
            if(orderside == 1){
                if(fg_oldimgcachei == 0){
                    fg_imgchangepreview(fg_imagesthumbs_cache[fg_imagesthumbs_cache.length-1].src);
                }else{
                    fg_imgchangepreview(fg_imagesthumbs_cache[fg_oldimgcachei-1].src);
                }
            }else{
                if(fg_oldimgcachei == fg_imagesthumbs_cache.length-1){
                    fg_imgchangepreview(fg_imagesthumbs_cache[0].src);
                }else{
                    fg_imgchangepreview(fg_imagesthumbs_cache[fg_oldimgcachei+1].src);
                }
