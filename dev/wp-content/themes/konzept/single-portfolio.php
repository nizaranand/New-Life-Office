<?php  get_header(); ?>
	...
	@media (min-width: 1000px) and (max-width: 1050px){ #header { opacity: 0; margin-top: -300px; } .imgscontainer { opacity: 0; } }
	@media (max-width: 750px){ #header { opacity: 0; margin-top: -300px; } .imgscontainer { opacity: 0; } }
	<?php } ?>
	p { margin: 0; }
	</style>
	<script type="text/javascript">
	jQuery(document).ready(function() {
//jQuery('body').mousedown(function(){ return false; });
});
</script>
	<!--[if gt IE 6]>
	<style type="text/css">
				...
				<div class="project-excerpt">
					<ul class="project-meta">
						<?php  if(get_post_meta($post->ID, 'portfolio_date', true)){ ?>
							<li class="project-date"><span class="project-meta-heading">DATE</span> <span class="page-exdate"><?php  echo get_post_meta($post->ID, 'portfolio_date', true); ?></span></li>
						<?php  } ?>
						<?php  if(get_post_meta($post->ID, 'portfolio_client', true)){ ?>
							<li class="project-client"><span class="project-meta-heading">CLIENT</span> <span class="page-exclient"><?php  echo get_post_meta($post->ID, 'portfolio_client', true); ?></span></li>
						<?php  } ?>
						<?php  if(get_post_meta($post->ID, 'portfolio_agency', true)){ ?>
							<li class="project-agency"><span class="project-meta-heading">AGENCY</span> <span class="page-exagency"><?php  echo get_post_meta($post->ID, 'portfolio_agency', true); ?></span></li>
						<?php  } ?>
						<?php  if(get_post_meta($post->ID, 'portfolio_ourrole', true)){ ?>
							<li class="project-ourrole"><span class="project-meta-heading">OUR ROLE</span> <span class="page-exourrole"><?php  echo get_post_meta($post->ID, 'portfolio_ourrole', true); ?></span></li>
						<?php  } ?>
					</ul>
...
<a href="https://twitter.com/share?url=<?php print(esc_url(get_permalink($portfoliopageidfl))); ?>&text=<?php print(urlencode($portfoliopagetitlefl)); ?>" class="twitter" style="color:<?php if($portfoliotextcolorfl=='#ffffff'){print("#ffffff");}else{print("#464646");} ?>;" title="Twitter">t</a>
<a href="http://www.facebook.com/sharer.php?u=<?php print(esc_url(get_permalink($portfoliopageidfl))); ?>&t=<?php print(urlencode($portfoliopagetitlefl)); ?>" class="facebook" style="color:<?php if($portfoliotextcolorfl=='#ffffff'){print("#ffffff");}else{print("#464646");} ?>;" title="Facebook">f</a>
<a href="https://plus.google.com/share?url=<?php print(esc_url(get_permalink($portfoliopageidfl))); ?>" style="color:<?php if($portfoliotextcolorfl=='#ffffff'){print("#ffffff");}else{print("#464646");} ?>;" class="googleplus" title="Google+">g</a>
</div>
