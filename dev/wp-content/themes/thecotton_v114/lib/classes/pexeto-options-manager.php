...
class PexetoOptionsManager{
	...
	var $pexeto_images_url='';
	var $pexeto_utils_url='';
	var $pexeto_uploads_url='';
	var $pexeto_version='';
	var $themename='';
	var $first_save='';
	 ...
	 */
	function PexetoOptionsManager($themename, $images_url, $utils_url, $uploads_url, $version){
		...
		$this->first_save=get_option(PEXETO_SHORTNAME.'_first_save');
	}
	...
	function print_upload($value){
		...
		echo '<script type="text/javascript">jQuery(document).ready(function($){
			pexetoOptions.loadUploader(jQuery("div#'.$value['id'].'_button"), "'.$this->pexeto_utils_url.'upload-handler.php", "'.$this->pexeto_uploads_url.'");
		});</script>'; 
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<script type="text/javascript">jQuery(document).ready(function($){
								pexetoOptions.loadUploader(jQuery("div#'.$field['id'].'_button"), "'.$this->pexeto_utils_url.'upload-handler.php", "'.$this->pexeto_uploads_url.'");
						});</script>';
					$preview=$field['id'];
					$is_textarea[]="false";
				break;
			...
			$saved_value=$this->get_field_value( $field['id'].'s',$field['std'] );
