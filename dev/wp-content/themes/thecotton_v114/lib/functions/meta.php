...
function pexeto_load_meta_boxes(){
	...
	$portf_categories=array(array('id'=>'-1', 'name'=>'All Categories'));
	...
	array(
		...
		create a new sidebar in "Dandelion Options->Sidebars" section and after that you will be able to select the
		sidebar here.'),
		
		array(
		"name" => "carousel",
		"title" => "Show portfolio projects in the bottom of the page",
		"type" => "select",
		"none" => true,
		"options" => $loader_portf_categories,
		"std" => 'hide',
		"description" => 'Displays a carousel(slider) containing the portfolio project images from the selected category.'),
		...
		"description" => 'The title of the portfolio projects carousel - available only when the "Show portfolio projects in the bottom of the page"
		...
		"type" => "text",
		"std" => "More Projects",
		"description" => "The title that is displayed on the right section on the Portfolio Showcase page template, just above the small projects preview"
		)
		);
...
<h4>'.$meta_box['title'].'</h4></div>';
		...
		case 'upload':
			...
			echo '<script type="text/javascript">jQuery(document).ready(function($){
				pexetoOptions.loadUploader(jQuery("div#pexeto-'.$meta_box['name'].'_button"), "'.PEXETO_UTILS_URL.'upload-handler.php", "'.PEXETO_UPLOADS_URL.'");
			});</script>'; 
