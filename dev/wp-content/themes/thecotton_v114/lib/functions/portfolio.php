 ...
 */
function pexeto_build_portfolio_image_html($post_id, $width, $height){
	$preview=get_post_meta($post_id, 'preview_value', true);
	$thumbnail=get_post_meta($post_id, 'thumbnail_value', true);

	if(!$thumbnail || $thumbnail==''){
		$thumbnail=pexeto_get_resized_image($preview, $width, $height);
	}
	...
	if($action=='nothing'){
		...
		$openLink='<a rel="lightbox" class="single_image" href="'.$preview.'" title="">';
		$closeLink='</a>';
	}

	return $openLink.'<img src="'.$thumbnail.'" class="shadow-frame" width="'.$width.'" height="'.$height.'" />'.$closeLink;
}