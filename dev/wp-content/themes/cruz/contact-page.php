...
if(isset($_POST['submit'])) {
	...
	if(!isset($errorFlag)) {	
		$to 		= $crz_email; //Replace this email address with yours
				...
				while (have_posts()) : the_post(); 
						...
						if(isset($sent)) { ?>
							<div id="mail_success_no_JS" class="box box2">
								<?php echo stripslashes($crz_success_msg); ?>
						...
						<p><input class="<?php if(isset($name_err) && $name_err != '') { ?>error<?php } ?>" id="name" name="username" value="<?php if(isset($_POST['username'])) echo $_POST['username'];?>" /><label for="name"><?php _e( 'Name*', 'cruz' ); ?></label></p>
						<p><input id="email" name="email" class="<?php if(isset($email_err) && $email_err != '') { ?>error<?php } ?>" value="<?php if(isset($_POST['email']))  echo $_POST['email'];?>" /><label for="email"><?php _e( 'E-mail*', 'cruz' ); ?></label></p>
						<p><input id="url" name="url" class="<?php if(isset($url_err) && $url_err != '') { ?>error<?php } ?>" value="<?php if(isset($_POST['url']))  echo $_POST['url'];?>" /><label for="url"><?php _e( 'URL', 'cruz' ); ?></label></p>                       
						<p><textarea name="comment" rows="10" cols="8" class="<?php if(isset($comment_err) && $comment_err != '') { ?>error<?php } ?>" id="comment" ><?php if(isset($_POST['comment'])) { if(function_exists('stripslashes')) { echo stripslashes($_POST['comment']); } else { echo $_POST['comment']; } } ?></textarea><label for="comment"><?php _e( 'Message*', 'cruz' ); ?></label></p>
						<p class="submit"><input name="submit" type="submit" class="submit" tabindex="5" value="<?php _e( 'Send Message', 'cruz' ); ?>" /></p>
						</form>
