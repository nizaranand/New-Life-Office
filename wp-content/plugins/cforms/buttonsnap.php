...
MORESCRIPT;
	...
	{
    	//Oliver:
		//return get_option('siteurl') . '/wp-c ontent/plugins/' . $this->basename($src);
        $t = defined('WP_CONTENT_URL') ? WP_CONTENT_URL . '/plugins/' . $this->basename($src) : get_option('siteurl') . '/wp-content/plugins/' . $this->basename($src);
        $t = defined('WP_PLUGIN_URL') ? WP_PLUGIN_URL .'/'. $this->basename($src) : $t;
        $t = defined('PLUGINDIR') ? get_option('siteurl') .'/'. PLUGINDIR . '/' . $this->basename($src) : $t;
		return $t;
...
if (!defined('ABSPATH')) {
	...
	### oliver: supporting WP2.6 wp-load
