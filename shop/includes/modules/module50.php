...
$page_content = '';
  ...
  $products_tag_array = rk_get_tag_products($tag_id, (int)$languages_id);
  $products_tag_list = implode(',', $products_tag_array);
  $products_new_array = array();

  $products_new_query_raw = "select p.products_id, p.products_model, pd.products_name,pd.products_description, p.products_quantity, p.products_image, p.products_weight, pd.products_name, p.products_price, p.products_tax_class_id, p.products_date_added, m.manufacturers_name from " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on (p.manufacturers_id = m.manufacturers_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id IN (".$products_tag_list.") and p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by p.products_date_added DESC, pd.products_name";
...
$page_content .= '<a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $products_new['products_id']) . '">' . IMAGE_BUTTON_BUY_NOW . '</a>'. ' : ' . $products_price .'<br />';
if (tep_not_null($products_new['manufacturers_name'])) $page_content .= TEXT_BY. $products_new['manufacturers_name'].'<br />'."\n";
if (tep_not_null($products_new['products_date_added'])) $page_content .= '<small>' .tep_date_long($products_new['products_date_added']) . '</small><br />'."\n";
if (tep_not_null($products_new['products_model'])) $page_content .= '<small>' . $products_new['products_model'] . '</small><br />'."\n";
$page_content .= '<small>' .short_name($products_new['products_description'], 180) . '</small>'."\n";
$page_content .= '</p>'."\n";
/// end description
$page_content .= '</div>'."\n";
$page_content .= '</div>'."\n";
...
$page_content .= osc_tag_cloud('number=0&echo=0'); /// display all tags
}
?>